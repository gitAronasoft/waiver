...
const CARD_W_MOBILE = 280;         // card width for mobile
const CARD_W_DESKTOP = 320;        // card width for desktop
const MOBILE_BP = 768;             // breakpoint for mobile view
...
// Show active or upcoming non-recurring events
...
// Show rule: weekly → only same weekday; else → active/upcoming
...
export default function EventsShowcase(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        // public endpoint that includes generated recurrences
        const {data}=await axios.get(`${BACKEND_URL}/events/public?horizon_days=60`);
  ...
  const handleComplete=()=> navigate("/all-done");
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"16px 16px 0" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <button onClick={()=>navigate(-1)} aria-label="Back"
                  style={{ background:"none", border:"none", fontSize:22, cursor:"pointer", padding:6 }}>←</button>
          <span style={{ fontWeight:600 }}>BACK</span>
        </div>
        <div style={{ textAlign:"center", marginTop:8 }}>
          <img src="/assets/img/SKATE_AND_PLAY_V08_Full_Transparency (2) 1.png" alt="logo" style={{ height:56, objectFit:"contain" }}/>
...
function Card({ ev, width, isActive, onClick, onBuy, isMobile }){
  ...
  const btnLabel = (ev.button_label ?? "").toString().trim() || "Réserver";
